...
import com.helipy.text.time.util.DateUtil;
...
/**
 * Pack:       com.helipy.text.time.nlp
 * File:       TimeNormalizerTest
 * Desc:
 * User:       chuangfeng.wang
 * CreateTime: 2024-10-30 17:35
 */
class TimeNormalizerTest {
     ...
     */
    @Test
    public void patternDebug() throws Exception {
        Pattern p = Pattern.compile(
                "" +
                        ...
                        "(((小学)|初中?|高中?|大学?|研(究生)?[一二三四五六七八九十]?(\\d+)?)?[上下]半?学期)|" +
                        "(([零一二三四五六七八九十百千万]+|\\d+)时期)|" +
                        "(午间)|(次年)|(这时候)|(农历新年)|" +
                        "([春夏秋冬](天|季))|" +
                        "((\\d+)天)|" +
                        ...
                        "((民主高中|我们所处的|复仇主义和其它危害人类的灾难性疾病盛行的|快速承包电影主权的|恢复自我美德|人类审美力基础设施|饱受暴力、野蛮、流血、仇恨、嫉妒的|童年|艰苦的童年)+时代)|" +
                        "(元旦)|" +
        ...
        String classPath = path.substring(0, path.indexOf("/com/helipy/text/time/nlp"));
        System.out.println(classPath);
        TimeNormalizer.writeModel(p, classPath + "/TimeExp0.zip");
    }
    ...
    public void end2end() throws Exception {
        ...
        TimeUnit[] unit = normalizer.parse("本周日到下周日出差");// 抽取时间
     ...
     */
    @Test
    public void exportRegText() throws Exception {
        String path = TimeNormalizer.class.getResource("").getPath();
        String classPath = path.substring(0, path.indexOf("/com/helipy/text/time/nlp"));
        System.out.println(classPath + "/TimeExp.m");
        TimeNormalizer normalizer = new TimeNormalizer(classPath + "/TimeExp.m");

        System.out.println(normalizer.getPatterns().pattern());
    }
}